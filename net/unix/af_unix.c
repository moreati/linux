...
static int unix_stream_connectat(struct socket *, struct path *, int flags);
...
static const struct proto_ops unix_stream_ops = {
	...
	.connectat =	unix_stream_connectat,
...
static struct sock *unix_find_other_at(struct net *net,
				    struct path *path, int type, int *error)
{
	struct inode *inode;
	struct sock *u;
	int err = 0;
...
static int unix_stream_connectat(struct socket *sock, struct path *path,
			         int flags)
{
	struct sock *sk = sock->sk;
	struct net *net = sock_net(sk);
...
restart:
	/*  Find listening sock. */
	other = unix_find_other_at(net, path, sk->sk_type, &err);
	if (!other)
		goto out;
